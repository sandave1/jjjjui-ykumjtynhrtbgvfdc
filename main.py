...
stopuser = {}
token = '7100170660:AAE5GkFVPw_LZ_nvNHo45dno4NwRhIrhnE0'
bot=telebot.TeleBot(token,parse_mode="HTML")
admin=[5504121561,114987882,2081963287,1039924357]
command_usage = {}
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/ddfmoto0")
			keyboard.add(contact_button)
			random_number = random.randint(33, 82)
			photo_url = f'https://t.me/chuslims_mc/256'
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/ddfmoto0")
		keyboard.add(contact_button)
		username = message.from_user.first_name
		random_number = random.randint(33, 82)
		photo_url = f'https://t.me/hitzzas/2'
...
𝘽𝙤𝙩 𝘽𝙮 @ddfmoto0''', reply_markup=mes)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='DD-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
...
def respond_to_vbv(message):
    bot.reply_to(message,f'''📍 Address Generator
𝗙𝘂𝗹𝗹 𝗡𝗮𝗺𝗲: {name} 
𝗦𝘁𝗿𝗲𝗲𝘁 𝗔𝗱𝗱𝗿𝗲𝘀𝘀: {street}
